...
- Supports built-in `build-classic`, `build-subagents`, and `plan` prompts plus inline, stdin, and file-based prompts; `build` is an alias that defaults to `build-classic`.
...
| Command | Behavior |
| --- | --- |
| `ralph` | Equivalent to `ralph run build` (`build` resolves via the `build-alias-prompt` config, default `build-classic`) |
...
The generated starter config disables logging by default (log file is empty). Users can enable logging during the interactive questionnaire by providing a log file path. The default prompts directory is set to `.ralph/prompts` so prompt files can live inside the repository. Init always writes `build-alias-prompt = "build-subagents"`, opting new projects into the batch build prompt.
...
- Built-in Prompts: `build-classic` (single-task build), `build-subagents` (batch build with at least one subagent per task), and `plan` with brief descriptions
- Aliases: `build` and the prompt name it resolves to (configurable via `build-alias-prompt`)
...
| `RALPH_BUILD_ALIAS_PROMPT` | Override the prompt name the `build` alias resolves to |
...
The built-in prompts use the placeholder `<COMPLETION_SIGNAL>`, which Ralph automatically replaces with `<promise>COMPLETE</promise>` at runtime before sending the prompt to the agent.
...
3. Run `ralph` or `ralph build` to implement the next task (the `build` alias defaults to `build-classic`; configs generated by `ralph init` target `build-subagents`).
